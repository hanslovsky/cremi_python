...
def run_evaluation():
    ...
    group_pattern     = '/volumes/predictions/neuron_ids-unlabeled-unmask-background' + \
                        '/%(setup)d/%(iteration)d/watersheds%(watersheds)s' + \
                        '/merge-threshold=%(merge_threshold)s_seed-distance=0.3_weight-distance-transform=0.1'
    ...
    parser.add_argument('--merge-threshold', type=str, choices=tuple(f'0.{d}' for d in range(1, 10, 2)), required=True)
    parser.add_argument('--glia-threshold', type=str, choices=tuple(f'0.{d}' for d in range(1, 10)), required=False)
    ...
    group                   = group_pattern % dict(
        ...
        watersheds      = '' if args.glia_threshold is None else ('-glia-threshold=%s' % args.glia_threshold),
    ...
    with z5py.File(container, 'r') as f:
        if 'wasSuccessful' not in f[group].attrs:
            print('No `wasSuccessful\' tag in group! Will not evaluate')
            raise Exception('No `wasSuccessful\' tag in group! Will not evaluate')
        elif not f[group].attrs['wasSuccessful']:
            print('`wasSuccessful\' tag in group is false! Will not evaluate')
            raise Exception('`wasSuccessful\' tag in group is false! Will not evaluate')
        # print(f[group].attrs['wasSuccessful'])
    ...
    evaluation_file_name = f'{container}{group}/{evaluation_base_path}.json'
